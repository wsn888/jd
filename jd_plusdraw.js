/**
入口：我的-plus专属-右边浮标
5 5,13 1-8 * * jd_plusdraw.js
*/
const $ = new Env('PLUS转盘抽豆');
...
async function main() {
  noplus = false;
  let actid = 'U13AR7JM6UNr2okmamEig7TD4Ef'
  let queryInteractive = await queryInteractiveInfo(actid)
  if ($.taskList) {
    for (const vo of $.taskList) {
        if (noplus) {
            console.log('不是plus会员，退出')
            return}
        if (vo.completionFlag === false && vo.assignmentType === 1) {
            console.log(`做任务：${vo.assignmentName}`);
...
function queryInteractiveInfo(projectId) {
  let body = { "encryptProjectId": projectId, "ext": {"rewardEncryptAssignmentId":null,"needNum":50} , "sourceCode": "aceaceqingzhan"}
...
function doInteractiveAssignment(projectId, encryptAssignmentId, itemId) {
  let body = { "encryptProjectId": projectId, "encryptAssignmentId": encryptAssignmentId, "sourceCode": "aceaceqingzhan", "itemId": itemId }
  return new Promise(resolve => {
    ...
    //$.log(data)
      try {
        if (err) {
          ...
          if (data) {
            data = JSON.parse(data);
            if (data.subCode == 1405){noplus = true}
...
function doreward(projectId) {
  let body = { "encryptProjectId": projectId, "encryptAssignmentId": "21qucmvBdAxp8A4NifVZmKxN11w1","completionFlag":true,"ext":{"exchangeNum":1},"sourceCode": "aceaceqingzhan"}
  return new Promise(resolve => {
    $.post(taskPostUrl("doInteractiveAssignment", body), async (err, resp, data) => {
    //$.log(data)
      try {
        if (err) {
          ...
          if (data) {
            data = JSON.parse(data);
            console.log(data.msg);
            //console.log(data.rewardsInfo.successRewards);
          } else {
            console.log("没有返回数据")
          }
        }
